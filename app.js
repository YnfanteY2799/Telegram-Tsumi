...
bot.start((ctx) =>{
    let {message:{from:{first_name, last_name}}} = ctx; 
    ctx.reply(`.... Starting Nii-sama`);
    ctx.reply(`.... Hi, im Tsumi`);
    ctx.reply(`Welcome ${first_name} ${last_name} nii-sama!`);
});

bot.command('giveRandomSauce',x => getRandomCode().then(ex => 
    x.reply(`${ex.code}, https://nhentai.net/g/${ex.code.replace("#","")}/`)
));

bot.command('getPopulars',x => getMainPageContentPopular().then(ex =>{
    x.reply(`The current most popular doujins are (${ex.length}) : \n` + ex.map(({code, link}) => 
    "\n" + `Code : ${code} ,` + "\n" + 
...
bot.command('download_code', act => {
    ...
    let {update:{message:{text} }} = act;
    text = text.split(" ")[1];

    bot.telegram.sendDocument(act.chat.id, {
        source:"./source/[Shimazu Tekko] Taiyou to Hiyakeato♥ Sun and Sunburn♥ (COMIC Kairakuten BEAST 2021-11) [English] [atang] [Digital].rar"
    });
...
bot.command('show_doujin', act => {
    ...
    getCodedDoujin(text)
    .then(({ogImages}) => ogImages.sort().map(x => act.reply(x)) )
...
bot.command('where_are_you', e => e.reply(`I'm somewhere near Tokyo!!!!, Just kidding i'm right here :\n https://github.com/YnfanteY2799/Telegram-Tsumi`))
